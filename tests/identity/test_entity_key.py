...
def test_from_locator_is_not_identity_stable():
    k = EntityKey.from_locator("loomweave:func:mod.foo")
    assert k.identity_stable is False
    assert k.value == "loomweave:func:mod.foo"


def test_from_sei_is_identity_stable():
    k = EntityKey.from_sei("loomweave:eid:01J")
    assert k.identity_stable is True
    assert k.value == "loomweave:eid:01J"


def test_locator_to_sei_is_a_value_swap_not_a_schema_change():
    loc = EntityKey.from_locator("loomweave:func:mod.foo")
    sei = EntityKey.from_sei("loomweave:eid:01J")
...
@pytest.mark.parametrize(
    "payload",
    [
        {"value": "loomweave:eid:01J", "identity_stable": "false"},
