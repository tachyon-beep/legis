...
> Correction (WP-A3, 2026-06-02): the row posted to Filigree is an UNSIGNED {entity_id, content_hash, actor} pointer; the tamper-binding lives in legis's dedicated signed BindingLedger (src/legis/governance/binding_ledger.py), verified on read (GET /signoff/{seq}/binding, forged → 500). The earlier wording overstated the Filigree-side binding as verdict-grade.
