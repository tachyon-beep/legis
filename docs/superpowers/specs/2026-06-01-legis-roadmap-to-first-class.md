...
`2026-06-01-loom-goal-state-case-study.md` (suite umbrella) and
...
> projects — **and** a first-class Loom citizen: an SEI-keyed, audit-grade
...
The **protected cell** is the elspeth CI judge pattern re-expressed in Loom's
...
- **Rename/history evidence.** The signal the SEI matcher needs: which symbols
  ...
  legis's eventual provider contribution to Clarion (§2.4) — but the surface
  must be built first.

The git interface is both a standalone capability (legis can answer "what
changed?") and the upstream dependency of every integration with Clarion.
...
and governance state of this project?"* — with no other Loom tool present.
...
those mechanisms, re-expressed in Loom's vocabulary — not elspeth-internal
...
- **One grammar, open instance set.** The grammar (what a policy boundary *is*,
  ...
  Same seam shape as Wardline's `TaintSourceProvider`, Clarion `Transport`, and
...
## 2. Half 2 — first-class Loom citizen
*(the layers; gated on siblings or on legis milestones 1–3)*

### 2.1 SEI-keyed attestations *(gated on Clarion shipping SEI)*
...
degrade gracefully when Clarion lacks the `sei` capability. Legis's half is a
consumer-layer change; the gate is Clarion implementing SEI.

Once SEI is available:

- Every verdict row carries an SEI as its entity key, not a locator.
- `lineage(sei)` from Clarion becomes legis's audit spine — the append-only
  ...
  every identity event since." Clarion's lineage is tamper-evidence-**able**;
...
- When an SEI is orphaned, legis surfaces a governance gap: "this entity had an
  ...
  relies on Clarion's existing §4 `resolve_sei` behavior (`alive: false,
...
- **Trust-vocabulary convergence.** One `@trust_boundary` grammar across the
  suite, delivering elspeth's custody and fabrication-test guarantees in Loom's
...
### 2.4 Git-rename signal provider to Clarion
...
Clarion's git code." Legis claims that provider seam.

This is not an identity-authority claim. Clarion remains the sole authority for
...
this locator") and Clarion's matcher consumes it. The seam design is legis's
concern; the identity decision is Clarion's.
...
| 3 | **Agent-programmable policy grammar** — grammar + builtins-as-defaults + in-code policy expression | none (sibling-independent) | greenfield; **highest-leverage un-gated item**; the hinge between "standalone tool" and "first-class Loom citizen" |
| 4 | **SEI-keyed attestations** — attestations key on SEI; consume lineage as audit spine; graceful degrade | Clarion ships SEI | consumer-layer change; thin & ready once milestones 1-3 exist; waits on sibling |
...
works across Python, Rust, TypeScript, or any language a Clarion plugin can
describe.
...
- **Degrade honestly** when Clarion does not advertise the `sei` capability:
...
identity is now orphaned; the attestation is in limbo") relies on Clarion's
...
consumes 'a git-rename signal,' not 'Clarion's git code.'" Legis formally
...
so the seam does not calcify as Clarion-internal before legis ships. This is a
...
at the governance boundary, not assumed from the store. Clarion's lineage is
...
Options (not prescriptive — legis raises the question for Clarion to decide):
...
2. Clarion guarantees the lineage endpoint is served from an append-only store
...
pre-lock requirement: record which option Clarion will implement so legis's
...
that sank Loom-URI, and the SEI standard's minimal-apparatus posture is
